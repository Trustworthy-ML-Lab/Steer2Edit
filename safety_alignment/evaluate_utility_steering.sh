...
for MODEL in "${MODELS[@]}"; do
  ...
  for DATASET in "${DATASETS[@]}"; do
    ...
    if [[ -f "$RESULT_JSON" ]]; then
      ...
      echo "[RUN ][Normal] evaluate_utility.py --model $MODEL --dataset $DATASET --n_sample $N_SAMPLE"
    ...
    for ALPHA in "${ALPHAS[@]}"; do
      ...
      echo "[RUN ][Steer ] evaluate_utility.py --model $MODEL --dataset $DATASET --alpha $ALPHA"
      python evaluate_utility.py \
        --model "$MODEL" \
        --dataset "$DATASET" \
        --n_sample "$N_SAMPLE" \
